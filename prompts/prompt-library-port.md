...
Before starting:
1. Confirm the project has a web server (not file:// protocol)
2. Identify where to place assets (e.g., /assets, /public, /static)
3. Identify where prompts are stored (e.g., /prompts, /docs/prompts)
4. Determine if a prompts index file exists or needs creation

────────────────────────────────
STEP 1 — Copy Drawer Assets

Copy these files from the source repository to the target project:

Source: /assets/prompt-drawer/prompt-drawer.js
Target: [project-assets-path]/prompt-drawer/prompt-drawer.js

Source: /assets/prompt-drawer/prompt-drawer.css
Target: [project-assets-path]/prompt-drawer/prompt-drawer.css

Maintain the same directory structure (prompt-drawer/ folder).

────────────────────────────────
STEP 2 — Create or Update Prompts Index

If the project has a prompts index:
- Update it to match the expected JSON structure
- Ensure it includes: id, title, category, description, file (relative path)

If the project does not have a prompts index:
- Create PROMPTS_INDEX.json in the prompts directory
- Structure: { "prompts": [ { "id": "...", "title": "...", "category": "...", "description": "...", "file": "/path/to/prompt.md" } ] }
- Add entries for all prompts that should be accessible

────────────────────────────────
STEP 3 — Update File Paths

In prompt-drawer.js, verify these paths match the target project structure:

1. Prompts index path (default: "/prompts/PROMPTS_INDEX.json")
   - Update if prompts are in a different location
   - Ensure path is relative to web root

2. Prompt file paths in index
   - Verify all "file" entries in PROMPTS_INDEX.json use correct relative paths
   - Paths should be relative to web root (start with "/")

────────────────────────────────
STEP 4 — Add to HTML Pages

Add these two lines to any HTML page where the drawer should be available:
...
If prompts fail to load:
- Check browser console for fetch errors
- Verify PROMPTS_INDEX.json path is correct
- Ensure JSON file is valid (no syntax errors)
- Confirm web server is serving files correctly

If drawer doesn't open:
- Check browser console for JavaScript errors
- Verify prompt-drawer.js is loaded (check Network tab)
- Ensure no JavaScript errors are blocking execution

If styles are missing:
- Verify prompt-drawer.css path is correct
- Check that CSS file is being served
- Ensure no CSS conflicts are overriding styles
