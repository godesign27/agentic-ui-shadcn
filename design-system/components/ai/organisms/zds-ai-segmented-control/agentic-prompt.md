# Agentic Prompt — AI Segmented Control

# AI Segmented Control — Agentic Prompt
...
- **Display name:** AI Segmented Control
- **Component id:** `zds-ai-segmented-control`
- **Category:** organisms
- **Status:** Beta
- **File path:** `src/app/components/ai/atomic/segmented-control/AISegmentedControl.tsx`
...
- An AI-branded alternative to the flat standard ZDS Segmented Control
- A control that must sit visually beside the AI Card, AI Tab, and AI Dialog

## When not to use
- You need the neutral product control — use the standard `zds-segmented-control`
- Options carry rich content panels — use `zds-ai-tab`
...
4. **Label** _(Shared)_ — @zsai-body-small — Medium default, SemiBold active.
...
| `--ai-zds-text` | `Primary text` | Label on inactive chips |
...
// Outline type, small size
<AISegmentedControl type="outline" size="small" value={view} onChange={setView}
  segments={[{ label: 'List', value: 'list' }, { label: 'Grid', value: 'grid' }]} />

// Full-width view switcher with a leading icon
<AISegmentedControl fullWidth value={view} onChange={setView}
  segments={[
    { label: 'List',  value: 'list',  leftIcon: 'list' },
    { label: 'Grid',  value: 'grid',  leftIcon: 'grid' },
