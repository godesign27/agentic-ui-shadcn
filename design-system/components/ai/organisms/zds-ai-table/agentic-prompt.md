...
- **Component id:** `zds-ai-table`
- **Category:** organisms
- **Status:** Stable
- **File path:** `src/app/components/zds/zds-table.tsx`
...
> ZDS Table, AI surface theme — brand-blue accent, brand-surface selection, rounded corners.

## Metadata
- **Category:** organisms
- **Status:** Stable
- **Source path:** `src/app/components/zds/zds-table.tsx`
...
- Any tabular data where the AI brand accent is preferred over the ZDS teal

## When not to use
- Don't use on a standard (non-AI) surface — use the standard <ZdsTable> (default theme) instead
- Don't fork zds-table.tsx to restyle — switch the theme prop
- Don't convey a cell state with color alone — pair with icon or text (AI modes use an inset edge)

## Anatomy
1. **Host** _(Unique)_ — ZdsTable theme="ai" — scoped region that re-anchors the atoms’ accent vars to the AI brand.
...
4. **Selection col** _(Shared)_ — ZDS Checkbox column; checked fill = var(--color-ai-brand). Selected row = brand-surface fill + brand inset edge.
5. **Action button** _(Shared)_ — Solid ZDS Button re-anchored to the AI brand blue (var(--color-ai-brand)).
...
| `columns` | `ZdsTableColumn[]` | `—` | Column defs (key, label, type, align, sortable, filterable, render…). |
...
| `rowMode` | `(row, i) => ZdsTableCellMode` | `—` | Row-level AI mode: 'ai-assist' \| 'ai-prefilled' \| 'ai-anomaly'. |
...
Standard ZDS Table structure with the AI surface theme.
- Import { ZdsTable } from components/zds/zds-table
...
## Code example
```tsx
import { ZdsTable, type ZdsTableColumn } from './components/zds/zds-table';

const columns: ZdsTableColumn[] = [
...
// For the un-themed ZDS teal table, omit theme (defaults to "standard").
```
