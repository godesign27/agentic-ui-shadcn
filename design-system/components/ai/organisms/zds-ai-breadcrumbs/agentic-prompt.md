...
- **Component id:** `zds-ai-breadcrumbs`
- **Category:** organisms
- **Status:** Stable
- **File path:** `src/app/components/zds/zds-breadcrumbs.tsx`
...
# AI Breadcrumbs
_zds-ai-breadcrumbs_

> ZDS Breadcrumbs, AI surface theme — brand-blue links, brand-ink current page.

## Metadata
- **Category:** organisms
- **Status:** Stable
- **Source path:** `src/app/components/zds/zds-breadcrumbs.tsx`
...
The AI-themed variant of the standard ZDS Breadcrumbs. The single ZdsBreadcrumbs component (zds/zds-breadcrumbs.tsx) accepts theme="standard" (ZDS primary teal links) or theme="ai". In the AI theme the link CSS variables are re-anchored — scoped via CSS-variable overrides — to the AI brand blue: links use var(--color-ai-brand), hover uses var(--color-ai-brand-strong), and the current page uses var(--color-ai-brand-ink). Everything else — sizes (Normal 16px / Small 14px / X-Small 12px), Default/Inverse styles, Default/Back types, the swappable zs-icon-home page icon with independent icon/text toggles, zs-icon-arrow-right separators, disabled/focus states, the 2px focus ring, and the "…" overflow collapse menu — is shared with the standard breadcrumbs. All colors, radii, and spacing read design-system CSS variables from globals.css; all text uses the "Open Sans" face.

## When to use
- Navigation trails on AI surfaces that should read as part of the AI experience
- Any breadcrumb where the AI brand accent is preferred over the ZDS teal
- Deep hierarchies (3+ levels) that benefit from a collapse menu

## When not to use
- Don't use on a standard (non-AI) surface — use the standard <ZdsBreadcrumbs> (default theme) instead
- Don't fork zds-breadcrumbs.tsx to restyle — switch the theme prop
...
Standard ZDS Breadcrumbs structure with the AI surface theme.
- Import { ZdsBreadcrumbs } from components/zds/zds-breadcrumbs
...
## Code example
```tsx
import { ZdsBreadcrumbs, type BreadcrumbItem } from './components/zds/zds-breadcrumbs';
...
<ZdsBreadcrumbs theme="ai" items={items} maxVisible={4} />

// Back variant:
<ZdsBreadcrumbs theme="ai" type="back" backLabel="Back to page" onBack={goBack} />

// For the un-themed ZDS teal breadcrumbs, omit theme (defaults to "standard").
```
