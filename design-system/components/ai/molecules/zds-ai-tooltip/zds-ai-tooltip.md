...
The ZDS Tooltip in AI clothing — same border-triangle arrows and popover geometry, AI brand indigo footer/confirmation buttons, and AI-softened corners (6px tooltips, 16px cards).

AITooltip is the AI-styled counterpart of the ZDS Tooltip system for AI surfaces (cards, drawers, generated dashboards). It is a re-skin, not a re-implementation: the tooltip / popover geometry, CSS border-triangle arrow math, size scale (Normal 16px / Small 14px), arrow directions (top / bottom / left / right) and positions (start / center / end), and the popover section structure are all ported verbatim from the ZDS Tooltip building blocks.

**Export:** `AITooltip`

Three things change on the AI brand ramp: (1) every text node uses the AI font token; (2) the hand-rolled teal footer / confirmation buttons are removed and replaced by the AI button atom (variant="primary" for Apply / Yes, variant="secondary" for Cancel / No, size="sm"), which carries AI brand indigo (#4D60E6); (3) corners are softened — hover tooltip bubbles use the small radius (6px) and the popover / confirmation cards use the medium radius (16px) with overflow clipping. Neutral surfaces (dark body #1A1628, white / subtle light surfaces) stay brand-neutral, matching the ZDS original.
...
- You want the ZDS tooltip behavior but AI brand styling

## When not to use

- The neutral product surface — use the standard `zds-tooltip`
- Don't embed forms or primary navigation — use a dialog for complex content
- Don't use for validation errors — use the ZDS validation / field component
...
2. **Arrow** _(Shared)_ — CSS border-triangle; direction (top / bottom / left / right) × position (start / center / end). Geometry inherited from ZDS.
...
4. **Footer buttons** _(Shared)_ — AIButton secondary (Cancel) + primary (Apply), size sm — AI brand indigo replaces the ZDS teal fills.
...
- **Popover · Inverse** _(position variants)_ — Assembled AIPopover with the four arrow directions at center. Elevation reuses the ZDS popover shadow stack.
...
| `--aiu-radius-xs` | `AI.radius.xs → 6px` | Hover tooltip bubble corner (was square in ZDS). |
| `--aiu-radius-md` | `AI.radius.md → 16px` | Popover + confirmation card corner (with overflow clip). |

### Tooltip surface & text (neutral, inherited from ZDS)
| Token | Value | Usage |
| --- | --- | --- |
...
### Shadow elevation (inherited from ZDS)
| Token | Value | Usage |
| --- | --- | --- |
...
1. Read this mirror spec and `zds-ai-tooltip.agent.json` before implementing.
2. Do not hardcode brand hex — use documented AI/ZDS tokens from `ai-tokens.ts`.
3. Do not invent dependency atoms — fetch canonical implementations from mirror specs.

Full agent contract: `components/ai/molecules/zds-ai-tooltip/zds-ai-tooltip.agent.json`.
