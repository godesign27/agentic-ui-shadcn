...
**Owner:** Zaidyn Design System  
**Depends On:** `components/ai/tokens/ai-tokens.ts`, `components/ai/tokens/color.md`  
**Used By:** AI surfaces, ZDS component hosts, shadcn/ui app shells  
...
|------|---------|----------------------|
| Tier 1 | `ZSAI[60]` | Palette overrides per theme variant (theme files only) |
...
| `css/zds-bridge.css` | Remaps `--zs-*` for standard ZDS components |
...
5. **ZDS still needs `.zs-master-style`.** The AI surface theme overrides token values inside that scope — it does not replace ZDS scoping.
...
|--------------|-------|-------------|
| `ai-default` | ZSAI Default | Light brand surface — ghost tint panels, blue-purple accents |
| `ai-bold` | ZSAI Bold | Dark brand surface — deep navy panels, lighter action accents |
| `ai-companion` | ZSAI Companion | Warm tan paper surface — companion ink and borders |
| `ai-aqua` | ZSAI Aqua | Teal-influenced page gradients — colors match `ai-default` |
...
<div data-theme="ai-default">
  <!-- AI, ZDS, and shadcn components inside read themed vars -->
</div>
```
...
<link rel="stylesheet" href="components/ai/tokens/css/zds-bridge.css">

<div class="zs-master-style" data-theme="ai-default">
  <!-- ZDS button, field, card inherit remapped --zs-* vars -->
</div>
```
...
- Do wrap AI/ZDS/shadcn surfaces in `data-theme` (or `SurfaceThemeProvider`).
...
- Don't replace `.zs-master-style` with `data-theme` — use both together for ZDS.
...
- [ ] ZDS hosts include `.zs-master-style` on an ancestor.
