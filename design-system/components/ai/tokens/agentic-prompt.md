...
You are implementing or consuming **Guild AI Design Tokens** from the mirror repo.
...
| | |
|---|---|
| **GitHub** | `the upstream AI component source` |
...
1. **Three tiers** — Tier 1 palettes (`AI_RAMP`, `SIGNAL_ORANGE`, `COMPANION_TAN`) are for token authors only. Components consume Tier 2 (`AI.*`) and Tier 3 (`--ai-*` CSS vars).
2. **Never hardcode brand hex** — use `AI.color.*`, `AI.gradient.*`, `var(--ai-card-bg)`, etc.
3. **Typography spreads** — `...AI_TYPOGRAPHY['@ai-*']` — no raw `fontSize` / `fontWeight`.
...
6. **Standard DS colors** live in `components/tokens/` — do not mix into AI component code except via documented bridges.
...
| File | Exports |
|------|---------|
| `ai-tokens.ts` | `AI`, `AI_RAMP`, `SIGNAL_ORANGE`, `COMPANION_TAN`, `DS`, `F`, `AI_THEME` |
...
- Do not reference `AI_RAMP[n]` directly in component code — use `AI.color.*`.
