...
**Date:** 2026-07-21
**Scope:** DS AI packages (`packages/ai-tokens`, `packages/ai-ui`, `packages/ai-ui-angular`)
...
- `SIGNAL_GREEN[60] = #27AE60`
- `SIGNAL_AMBER[60] = #E67E22`

### Tier 2 — Semantic aliases (`ai-tokens.ts`)
- `AI.color.status.success = SIGNAL_GREEN[60]`
- `AI.color.status.warning = SIGNAL_AMBER[60]`
...
- `packages/ai-tokens/src/ai-typography.ts`: added `@ai-metric-value-md` (26/700/1.1) and `@ai-metric-value-lg` (34/700/1.1) — large numeric display scale.
- **NEW FILE** `packages/ai-tokens/src/css/ai-typography.css` — mirrors `AI_TYPOGRAPHY` font sizes as CSS vars for Angular/CSS consumers:
  `--ai-ramp-font-size-{12,14,15,16,20,22,24,26,32,34,48,64}`.
...
- React `AIChipBrief` status map → `--zs-surface-*`/`--zs-border-*`/`--zs-text-*` + `--ai-ramp-*`.
...
- **181** `font-size` literals at existing scale sizes → `var(--ai-ramp-font-size-N, Npx)` (no visual change).
- **197** off-scale literals **normalized** to the nearest token (visual change — intended):
  - `9px, 10px, 11px → 12px` (`--ai-ramp-font-size-12`) — 12px floor (typography.md §2)
  - `13px → 14px` (`--ai-ramp-font-size-14`) — sub-14 tier fix (§3)
  - `18px → 20px` (`--ai-ramp-font-size-20`)
...
### D. React typography
- `AIMessageHeader` (label → `@ai-agent-name`, timestamp → `@ai-caption-1`), `AIConfidenceRiskBadge` compact → `@ai-caption-2`, `AIChipBrief` sm → `@ai-caption-2`.
...
2. **Load the new type-scale layer** on the doc site (import `@guild/ai-tokens/ai-typography.css` after `ai-surface.css` + `ai-component-tokens.css`). Until loaded, the px fallbacks render identically — but the `--ai-ramp-font-size-*` vars won't be themable.
...
4. **Update component docs / `.agent.json` manifests**:
   ...
   - Note reduced-motion behavior and the normalized type scale (`--ai-ramp-font-size-*` + `@ai-metric-value-md/lg`).
