...
/**
 * ZDS Radio Button Component
 * ==========================
 * Source of Truth: zsainc/9904PD0068_zds-ai-mirror → src/radio/ (radio.less)
 *
 * Circular selection control for mutually-exclusive choices. Structurally a
 * sibling of ZdsCheckbox — same token names, sizing, focus ring, error and
 * disabled treatment — but round with a filled center dot when selected.
 */

export const ZDS_RADIO_SOURCE = {
  github: 'src/radio/',
  mappings: {
...
export type ZdsRadioSize = '14px' | '16px' | '18px' | '20px';
export type ZdsRadioMode = 'default' | 'error';

export interface ZdsRadioProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size'> {
  size?: ZdsRadioSize;
  mode?: ZdsRadioMode;
  label?: React.ReactNode;
}

export function ZdsRadio({
  size = '16px',
  checked = false,
  disabled = false,
  mode = 'default',
  ...
  return (
    <label
      className={(className ? className + ' ' : '') + 'zds-radio-wrapper'}
    ...
    >
      <style dangerouslySetInnerHTML={{ __html: `
        .zds-radio-wrapper input:focus-visible + div {
