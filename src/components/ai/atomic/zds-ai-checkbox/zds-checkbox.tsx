...
/**
 * ZDS Checkbox Component
 * =====================
 * Source of Truth: zsainc/9904PD0068_zds-ai-mirror → src/checkbox/
 */

export const ZDS_CHECKBOX_SOURCE = {
  github: 'src/checkbox/',
  mappings: {
...
export type ZdsCheckboxSize = '14px' | '16px' | '18px' | '20px';
export type ZdsCheckboxMode = 'default' | 'error';

export interface ZdsCheckboxProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size'> {
  size?: ZdsCheckboxSize;
  indeterminate?: boolean;
  mode?: ZdsCheckboxMode;
  label?: React.ReactNode;
}

export function ZdsCheckbox({
  size = '16px',
  checked = false,
  indeterminate = false,
  disabled = false,
  ...
  return (
    <label
      className={className + " zds-checkbox-wrapper"}
    ...
    >
      <style dangerouslySetInnerHTML={{__html: `
        .zds-checkbox-wrapper input:focus-visible + div {
