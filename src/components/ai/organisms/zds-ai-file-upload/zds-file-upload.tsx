...
/**
 * Theme layer. The standard 'standard' theme keeps the ZDS primary teal
 * (var(--primary) #2F6F7B) as the brand accent and renders the hardcoded square
 * ZDS button + teal progress bar. The 'ai' theme is the AI Surface Theme applied
 ...
 * themes use the same inline-SVG ZDS iconography below.
 */
export type ZdsFileUploadTheme = 'standard' | 'ai';

export type ZdsFileUploadSize = 'normal' | 'small' | 'xsmall';
export type ZdsFileUploadState = 'in-progress' | 'completed' | 'success' | 'error';
export type ZdsUploadedFileState = 'in-progress' | 'default' | 'success' | 'error';
export type ZdsUploadContainerState = 'default' | 'active' | 'disabled';

export interface ZdsUploadedFile {
  id: string;
  name: string;
  size: string;
  state?: ZdsUploadedFileState;
...
// ── Tokens (globals.css) ──────────────────────────────────────────────────────
// Only the brand accent differs by theme — standard = ZDS primary teal
...
function tokensFor(theme: ZdsFileUploadTheme) {
...
// ── Icons ─────────────────────────────────────────────────────────────────────
// Canonical ZDS iconography rendered as inline SVG. The zsIcons web-font shipped
...
//   zsainc/9904PD0068_zds-ai-mirror @ 2f64002 · src/core/SVGs/…
...
function ZsSvgIcon({ size, color, path }: { size: number; color: string; path: string }) {
  return (
    <svg
      width={size}
      height={size}
...
const UploadFileIcon   = ({ size, color }: { size: number; color: string }) => <ZsSvgIcon size={size} color={color} path={ZDS_ICON_PATHS['upload-file-fill']} />;
const DocIcon          = ({ size, color }: { size: number; color: string }) => <ZsSvgIcon size={size} color={color} path={ZDS_ICON_PATHS['doc-generic']} />;
const CheckCircleIcon  = ({ size, color }: { size: number; color: string }) => <ZsSvgIcon size={size} color={color} path={ZDS_ICON_PATHS['check-circle-fill']} />;
const ErrorHexagonIcon = ({ size, color }: { size: number; color: string }) => <ZsSvgIcon size={size} color={color} path={ZDS_ICON_PATHS['error-hexagon-fill']} />;
const DeleteIcon       = ({ size, color }: { size: number; color: string }) => <ZsSvgIcon size={size} color={color} path={ZDS_ICON_PATHS['delete']} />;

// ── Progress bar ──────────────────────────────────────────────────────────────
// Standard theme: filled teal fill + 70%-opacity neutral track (the ZDS spec).
...
}: { value: number; height: number; label?: string; theme: ZdsFileUploadTheme }) {
...
// ── Select File button ────────────────────────────────────────────────────────
// Standard theme: the square ZDS button (0 radius per spec). AI theme: the real
// AIButton atom (primary-solid = flat AI brand color) — never a hardcoded copy.
const AI_BTN_SIZE: Record<ZdsFileUploadSize, 'sm' | 'md'> = { normal: 'md', small: 'sm', xsmall: 'sm' };

function SelectFileButton({
  label, size, disabled, onClick, theme,
}: { label: string; size: ZdsFileUploadSize; disabled?: boolean; onClick?: () => void; theme: ZdsFileUploadTheme }) {
  if (theme === 'ai') {
    return (
...
// ── Drop-zone container ───────────────────────────────────────────────────────
export interface ZdsFileUploadContainerProps {
  size?: ZdsFileUploadSize;
  state?: ZdsUploadContainerState;
  helperText?: boolean;
  primaryText?: string;
  helper?: string;
  buttonLabel?: string;
  onSelect?: () => void;
  theme?: ZdsFileUploadTheme;
}

export function ZdsFileUploadContainer({
  size = 'normal',
  state = 'default',
  helperText = true,
...
// ── Uploaded file row ─────────────────────────────────────────────────────────
export interface ZdsUploadedFileRowProps {
  file: ZdsUploadedFile;
  size?: ZdsFileUploadSize;
  onRemove?: (id: string) => void;
  theme?: ZdsFileUploadTheme;
}

export function ZdsUploadedFileRow({ file, size = 'normal', onRemove, theme = 'standard' }: ZdsUploadedFileRowProps) {
  const C = tokensFor(theme);
  const S = SIZES[size];
...
// ── Full assembly ─────────────────────────────────────────────────────────────
export interface ZdsFileUploadProps {
  /** Overall variant; when `files` is omitted this seeds a representative list. */
  state?: ZdsFileUploadState;
  single?: boolean;
  size?: ZdsFileUploadSize;
  containerState?: ZdsUploadContainerState;
  helperText?: boolean;
  primaryText?: string;
  helper?: string;
  buttonLabel?: string;
  ...
  files?: ZdsUploadedFile[];
  width?: number | string;
  onSelect?: () => void;
  onRemove?: (id: string) => void;
  /** 'standard' = ZDS primary teal + square button; 'ai' = AI brand + AIButton/AIProgress. */
  theme?: ZdsFileUploadTheme;
}

// Map the full-component State to the per-row state and seed demo rows.
function seedFiles(state: ZdsFileUploadState, single: boolean): ZdsUploadedFile[] {
  const rowState: ZdsUploadedFileState =
      ...
      {rows.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 20 }}>
            {rows.map((f) => (
              <ZdsUploadedFileRow key={f.id} file={f} size={size} onRemove={onRemove} theme={theme} />
            ))}
          </ul>
        </div>
      )}
