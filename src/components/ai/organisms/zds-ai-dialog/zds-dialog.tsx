...
/**
 * ZDS Dialog Component
 * =====================
 * Source of Truth: zsainc/9904PD0068_zds-ai-mirror → src/dialog/
 * (dialog.less, dialog.stories.js, ZDS-DIALOG.agent.md).
...
export const ZDS_DIALOG_SOURCE = {
  github: 'src/dialog/',
  mappings: {
...
export type ZdsDialogHeaderStyle = 'default' | 'dark';
export type ZdsDialogSize = 'small' | 'medium' | 'large';
export type ZdsDialogAlignment = 'left' | 'center';
export type ZdsDialogOverlayColor = 'default' | 'inverse';
export type ZdsDialogOverlayOpacity = 90 | 60 | 40 | 20;
export type ZdsDialogFooterButtons = 'one' | 'two';

export interface ZdsDialogProps {
  isOpen: boolean;
  /** Header treatment. `dark` darkens ONLY the header bar (Dark Header Modal). */
  header?: ZdsDialogHeaderStyle;
  size?: ZdsDialogSize;
  alignment?: ZdsDialogAlignment;
  hasFooter?: boolean;
  hasHelperText?: boolean;
  /** Backdrop color: white 90% (default) or dark (inverse). */
  overlayColor?: ZdsDialogOverlayColor;
  /** Backdrop opacity variant. */
  overlayOpacity?: ZdsDialogOverlayOpacity;
  /**
   ...
   */
  footerButtons?: ZdsDialogFooterButtons;
  title: string;
  helperText?: string;
...
export function ZdsDialog({
  ...
  const scopeId = useRef(`zds-dialog-${Math.random().toString(36).slice(2, 9)}`).current;
  ...
  const buttonFooter =
    footerButtons === 'two' ? (
      <>
        <ZdsButton type="outline" size="normal" onClick={onClose} label="Option1" />
        <ZdsButton type="solid" size="normal" onClick={onClose} label="Option2" />
      </>
    ) : footerButtons === 'one' ? (
      <ZdsButton type="solid" size="normal" onClick={onClose} label="Button" />
    ) : null;
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        #${scopeId} .zds-dialog-close:focus-visible {
          box-shadow: 0 0 0 2px ${headerBg}, 0 0 0 4px var(--zs-border-focus, #4d60e6) !important;
          outline: none;
        }
        #${scopeId} .zds-dialog-body::-webkit-scrollbar {
          width: 8px;
          background: var(--zs-scrollbar-background, #dedcde);
        }
        #${scopeId} .zds-dialog-body::-webkit-scrollbar-thumb {
      ...
      <dialog
        id={scopeId}
        ref={dialogRef}
        className={`zds-dialog ${className || ''}`}
        aria-modal="true"
      ...
      >
        ...
        {/* Body */}
        <div className="zds-dialog-body" style={{
          padding: '16px',
          overflowY: 'auto',
          flex: 1,
          textAlign: centered ? 'center' : 'left',
