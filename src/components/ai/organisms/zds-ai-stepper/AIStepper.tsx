...
import { ZDS_ICON_PATHS } from '../../atomic/ai-icon/AIIcon';

/**
 * AIStepper — the ZDS Compact Stepper, re-imagined in the ZAIDYN AI brand.
 *
 * This is the AI-surface sibling of the standard ZDS Stepper
 * (src/app/components/ai-library/standardEntries/phaseH.tsx → CompactStepper,
 * itself a 1:1 port of the canonical `zs-stepper zs-compact-stepper` markup).
 ...
 *
 * Status badges keep the ZDS semantic meaning but pull their color from the AI
 * status set (success / warning / error) and the AI brand for info / current.
 *
 * Consumes AI tokens only (ai-tokens.ts) plus the shared ZDS neutral aliases for
 ...
 * shared ZDS_ICON_PATHS registry (the zsIcons *font* only resolves in Angular,
...
const PENDING_NUM    = ZDS.textDisabled;       // #716e79 — pending number text
const PENDING_DOT    = AI.color.brandBorder;   // #BECAFE — pending dot fill
const MUTED_TEXT     = ZDS.textDisabled;       // #716e79 — muted label after current
const HELP_TEXT      = ZDS.textHelper;         // #5b5864 — sub-line
...
export interface AIStepperProps {
   ...
   *   'neutral'         — active pill + filled circles use ZDS neutral greys.
...
// neutral pulls the shared ZDS neutral tokens (no brand hue).
...
const INLINE_TONES: Record<'brand' | 'neutral', InlinePalette> = {
  ...
  neutral: {
    fill:       ZDS.textDefault,      // #2f2c3c — solid neutral disc
    fillText:   ZDS.textInverse,      // #ffffff — knocked-out number/glyph
    restFill:   ZDS.borderSubtle,     // #d5d3d8 — pending disc
    restText:   ZDS.textDisabled,     // #716e79 — pending number
    pillBorder: ZDS.border,           // #B2B0B6 — neutral pill outline
    pillGlow:   undefined,            // no coloured glow in neutral tone
    track:      ZDS.borderSubtle,     // #d5d3d8 — neutral connector
  },
};
...
// Render ZDS glyphs as inline SVG (24×24, currentColor). The zsIcons *font*
// (zs-icon-* classes) only resolves in Angular; in React we draw the path data
// directly from the shared ZDS_ICON_PATHS registry so glyphs always render.
function Icon({ name, size, color }: { name: string; size: number; color?: string }) {
  const glyph = ZDS_ICON_PATHS[name];
  if (!glyph) {
 ...
 * stepper in the ZDS neutral tone. Copy/paste this alongside AIStepper to drop
