 ...
 * DS Button Component
 * =====================
 * Source of Truth: upstream AI component source → src/button/
 */

export const DS_BUTTON_SOURCE = {
  github: 'src/button/',
  mappings: {
...
export type DSButtonType = 'solid' | 'outline' | 'link';
export type DSButtonSize = 'normal' | 'small' | 'xsmall';
export type DSButtonStyle = 'icon-left' | 'text-only' | 'circle' | 'square' | 'icon-only';

export interface DSButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  type?: DSButtonType;
  size?: DSButtonSize;
  btnStyle?: DSButtonStyle; // avoiding `style` conflict with React.CSSProperties
