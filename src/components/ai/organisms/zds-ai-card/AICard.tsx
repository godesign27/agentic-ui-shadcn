...
import { ZdsCard, ZdsCardProps } from './zds-card';
import { AI } from '../../tokens/ai-tokens';

/**
 * ZDS AI Card
 * ===========
 * The **real** standard `ZdsCard` with the **AI surface theme** applied.
 *
 * `ZdsCard` is driven entirely by CSS custom properties (--radius, --border,
 ...
 *   header background               → #FFFFFF / #FAFAFA       (headerTone — scoped !important, header bg is inline in ZdsCard)
...
// Was ZSAI[40] (#A6B4FC). Moved onto the semantic token; brandBorder is one
...
// ─── Metadata export (mirrors ZDS_CARD_SOURCE) ───────────────────────────────
export const ZDS_AI_CARD_SOURCE = {
  base: 'zds/zds-card.tsx (ZdsCard)',
...
// Scoped class so the header gray + rounded clip apply only inside ZdsAiCard.
const SCOPE_CLASS = 'zds-ai-card-scope';
// ZdsCard sets the header background inline (var(--background)), so a gray header
...
export type ZdsAiCardBorderTone = 'neutral' | 'brand';
export type ZdsAiCardHeaderTone = 'body' | 'neutral';

export interface AICardProps extends Omit<ZdsCardProps, 'body' | 'header' | 'footer' | 'hasHeader' | 'hasFooter'> {
  /** Resting border color: 'neutral' gray (default) or 'brand' AI blue. */
  borderTone?: ZdsAiCardBorderTone;
  /** Header fill: 'body' white (default) or 'neutral' gray. */
  headerTone?: ZdsAiCardHeaderTone;
  hasHeader?: boolean;
  hasFooter?: boolean;
...
export function AICard({
  ...
  return (
    <div className={SCOPE_CLASS} data-header={headerTone} style={themeVars}>
      ...
      <ZdsCard {...props} hasHeader={hasHeader} hasFooter={hasFooter} header={header} body={body} footer={footer} />
    </div>
  );
}
