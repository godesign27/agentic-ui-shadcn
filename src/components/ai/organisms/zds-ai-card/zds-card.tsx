 ...
 * ZDS Card Component
 * =====================
 * Source of Truth: zsainc/9904PD0068_zds-ai-mirror → src/card/
 *
 * Canonical LESS → CSS variable mappings (card.less + config.m.js):
 *   @zs-card-background-color         → var(--background)           #ffffff
 *   @zs-card-hover-background-color   → var(--surface-color-1)      #FAFAFA  (palette.neutral[0])
 *   @zs-card-focus-background-color   → var(--surface-color-1)      #FAFAFA
 *   @zs-card-active-background-color  → var(--teal-00)              #FEFFFF  (palette.primary[0])
 *   @zs-card-active-border-color      → var(--primary)              #2F6F7B  (palette.primary[80] = teal-80)
 *   @zs-card-border                   → 1px solid var(--border)     #B2B0B6  (palette.neutral[30])
 *   @zs-card-border-radius            → var(--radius)               0px
 *   @zs-card-box-shadow               → var(--shadow-flat-right-angle-2)
 *   @zs-card-hover-box-shadow         → var(--shadow-flat-right-angle-4)
...
export const ZDS_CARD_SOURCE = {
  github: 'zsainc/9904PD0068_zds-ai-mirror/src/card/',
  lessFile: 'src/card/card.less',
  configFile: 'src/card/config.m.js',
  mappings: {
    '@zs-card-background-color':        'var(--background)',
    '@zs-card-hover-background-color':  'var(--surface-color-1)',
    '@zs-card-focus-background-color':  'var(--surface-color-1)',
    '@zs-card-active-background-color': 'var(--teal-00)',
    '@zs-card-active-border-color':     'var(--primary)',
    '@zs-card-border':                  '1px solid var(--border)',
    '@zs-card-border-radius':           'var(--radius)',
    '@zs-card-box-shadow':              'var(--shadow-flat-right-angle-2)',
    '@zs-card-hover-box-shadow':        'var(--shadow-flat-right-angle-4)',
...
// ─── Types ────────────────────────────────────────────────────────────────────
export type ZdsCardMode = 'light' | 'dark';

export interface ZdsCardProps {
...
export function ZdsCard({
  ...
  const rootStyle: React.CSSProperties = {
    ...
    color:          textColor,
    borderRadius:   'var(--radius)',           // 0px — @zs-card-border-radius
    ...
    textDecoration: 'none',
    outline:        'none',
    // Matches card.less: transition border + box-shadow @zs-animation-duration-fast ease-out
    ...
    <>
      ...
      {isClickable && (
        <style dangerouslySetInnerHTML={{ __html: `
          .zds-card-interactive:focus-visible {
      ...
      <Container
        id={id}
        className={[className, isClickable ? 'zds-card-interactive' : ''].filter(Boolean).join(' ') || undefined}
