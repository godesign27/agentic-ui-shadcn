...
import { ZdsCheckbox } from './zds-checkbox';
import { ZdsToggle } from './zds-toggle';
import { ZdsButton } from './zds-button';
import { ZdsBadge } from './zds-badge';
 ...
 * ZDS Table
 * =========
 * Source of Truth: zsainc/9904PD0068_zds-ai-mirror → src/table/
 *   (ZDS-TABLE.agent.md · zds-table.html canonical markup · table.md spec)
 * Build brief: src/imports/pasted_text/table-component-build.md
 *
 * A full-featured, data-driven data table assembled from existing ZDS atoms
...
// ─── Token contract (ZDS-TABLE.agent.md → mapped onto globals.css vars) ───────
// The table ships in two design themes, both driven entirely by globals.css vars:
//   • 'standard' — the ZDS system look: primary TEAL accent (var(--zs-selection-
...
export type ZdsTableTheme = 'standard' | 'ai';

function getTokens(theme: ZdsTableTheme) {
  const ai = theme === 'ai';
  return {
    ...
    // Accent: AI brand blue for the AI theme, ZDS primary teal for standard.
    ...
    // Rounded AI corners vs. squared ZDS corners.
...
export type ZdsTableSize = 'normal' | 'small';
export type ZdsTablePadding = 'default' | 'generous';
export type ZdsTableSelection = 'none' | 'single' | 'multi';
export type ZdsTableBorder = 'bottom-on-row' | 'left-on-cell' | 'all' | 'none';
export type ZdsTableCellMode = 'default' | 'ai-assist' | 'ai-prefilled' | 'ai-anomaly';
export type ZdsTableColumnType =
...
const DEFAULT_WIDTH: Partial<Record<ZdsTableColumnType, number>> = {
  checkbox: 52, radio: 52, action: 52, toggle: 64,
  'short-text': 160, number: 160, 'text-with-icon': 180,
  'long-text': 300, date: 215, datetime: 225,
};

export interface ZdsTableColumn<Row = any> {
  key: string;
  label?: string;
  type?: ZdsTableColumnType;
  align?: 'left' | 'right';
  sortable?: boolean;
  filterable?: boolean;
  ...
  render?: (row: Row, col: ZdsTableColumn<Row>) => React.ReactNode;
  /** Per-cell AI mode (overrides the row-level mode). */
  mode?: (row: Row) => ZdsTableCellMode;
}

export interface ZdsTableProps<Row = any> {
  columns: ZdsTableColumn<Row>[];
  rows: Row[];
  rowKey?: (row: Row, i: number) => string | number;
  /** Visual theme: 'standard' = ZDS teal / square corners; 'ai' = AI brand blue / rounded. */
  theme?: ZdsTableTheme;
  size?: ZdsTableSize;
  padding?: ZdsTablePadding;
  selection?: ZdsTableSelection;
  selectedKeys?: (string | number)[];
  onSelectionChange?: (keys: (string | number)[]) => void;
  sortable?: boolean;
  sort?: { key: string; dir: SortDir };
  onSortChange?: (sort: { key: string; dir: SortDir }) => void;
  border?: ZdsTableBorder;
  zebra?: boolean;
  headerColor?: boolean;
  ...
  caption?: string;
  /** Row-level AI mode. */
  rowMode?: (row: Row, i: number) => ZdsTableCellMode;
  ...
  onButtonClick?: (row: Row, col: ZdsTableColumn<Row>) => void;
...
function rowMetrics(size: ZdsTableSize, padding: ZdsTablePadding) {
...
function modeStyles(mode: ZdsTableCellMode, T: TableTokens): React.CSSProperties {
  switch (mode) {
    case 'ai-assist':
...
export function ZdsTable<Row = any>({
  columns,
  rows,
  rowKey,
  theme = 'standard',
  ...
  const cycleSort = (col: ZdsTableColumn<Row>) => {
  ...
  const renderCellContent = (row: Row, col: ZdsTableColumn<Row>, rowIdx: number): React.ReactNode => {
    ...
    switch (col.type) {
      ...
      case 'boolean':
        ...
        // theme keeps the ZDS toggle (primary teal).
        return isAi ? (
          <AIToggle
            checked={!!val}
            size="sm"
          ...
          >
            <ZdsToggle active={!!val} size="small" />
          </span>
        );
      case 'action':
        return (
      ...
      case 'button':
        ...
        // standard theme keeps the ZDS solid teal button (square corners).
        return isAi ? (
          <AIButton
      ...
      case 'badge':
        return <ZdsBadge variant={col.badgeVariant ? col.badgeVariant(row) : 'neutral'} label={String(val ?? '')} />;
      case 'text-with-icon':
        return (
  ...
  const scoped = `
    .zds-table-row:hover:not([aria-disabled="true"]) { background: ${TOKENS.bgHover} !important; }
    @keyframes zdsTableShimmer { 0% { opacity: .55 } 50% { opacity: 1 } 100% { opacity: .55 } }
    .zds-table-skeleton { animation: zdsTableShimmer 1.4s ease-in-out infinite; }
  `;
  ...
  return (
    <div
      ...
      style={{
        fontFamily: FONT,
        // AI theme only: retarget the reused ZDS atoms' accent tokens (checkbox
        ...
        // 'standard' theme leaves these untouched, so the atoms keep their ZDS
