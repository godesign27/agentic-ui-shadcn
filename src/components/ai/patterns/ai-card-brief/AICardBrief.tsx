  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <AIAvatar size={28} />
        <div style={{ flex: 1 }}>
          <div style={{ ...AI_TYPOGRAPHY['@zsai-section-subtitle'], color: 'var(--ai-zds-text)' }}>{agentName}</div>
          <div style={{ ...AI_TYPOGRAPHY['@zsai-agent-name'], color: 'var(--ai-zds-helper)' }}>Task Brief</div>
        </div>
        <AIChip kind="brief" status={status} label={status.replace(/-/g, ' ')} size="sm" />
        <button
          onClick={() => setExpanded(e => !e)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ai-zds-helper)', padding: 4 }}
        >
        ...
        <>
          <div style={{ padding: '18px 20px 20px', display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            <div>
              ...
              <div style={{ ...AI_TYPOGRAPHY['@zsai-body'], color: 'var(--ai-zds-text)' }}>{goal}</div>
            </div>
            ...
            {constraints.length > 0 && (
              <div>
                ...
                <div style={{ ...AI_TYPOGRAPHY['@zsai-body-small'], color: 'var(--ai-zds-text)' }}>
                  {constraints.map((c, i) => (
                    <React.Fragment key={i}>
                      {i > 0 && (
                        <span aria-hidden="true" style={{ margin: '0 8px', color: 'var(--ai-zds-helper)' }}>·</span>
                      )}
                      {c}
                    </React.Fragment>
                  ))}
            ...
            {outputFormat && (
              <div>
                ...
                <div style={{ ...AI_TYPOGRAPHY['@zsai-body-small'], color: 'var(--ai-zds-text)' }}>{outputFormat}</div>
              </div>
            )}
          </div>
          ...
          <div style={{
            ...
            {onSave && (
              <button onClick={onSave} style={{ ...BTN_BASE, background: 'transparent', color: 'var(--ai-zds-helper)', border: '1px solid var(--ai-btn-outline-border)' }}>
                Save
              </button>
            )}
            {onCancel && (
              <button onClick={onCancel} style={{ ...BTN_BASE, background: 'transparent', color: 'var(--ai-zds-helper)', border: 'none' }}>
                Cancel
              </button>
            )}
          </div>
