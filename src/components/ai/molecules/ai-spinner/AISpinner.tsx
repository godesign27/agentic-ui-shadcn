 ...
 * AISpinner
 *
 * The canonical DS spinner (../ds/ds-spinner) re-skinned with the AI brand
 * surface. This does NOT fork the SVG — it wraps <DSSpinner /> and re-anchors
...
import { DSSpinner, type DSSpinnerProps } from './ds-spinner';
import { AI, AI_RAMP } from '../../tokens/ai-tokens';

export interface AISpinnerProps extends DSSpinnerProps {
...
export function AISpinner({ dark = false, inverse = false, ...rest }: AISpinnerProps) {
  ...
  if (inverse) {
    ...
    // Deliberate Tier-1 exception. One ramp up from surface.emphasis (AI_RAMP[30]) →
